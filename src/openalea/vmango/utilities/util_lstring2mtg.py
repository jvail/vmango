...
def export_to_mtg(lstring):
  ...
  mtg = axialtree2mtg(lstring,
                      ...
                      scene = None,
                      parameters = dict(list(zip(modules_to_export,params_to_export))))
...
def export_to_mtg_light(lstring, cycle):
  ...
  toupdate  = [(v,'Inflorescence') for v,n in list(labels.items()) if n == 'InflorescenceBud' and mtg.property('cycle')[v] == cycle]
