...
def get_simulated_mtg_repository(variety = 'cogshall'):
    return join(share_dir, 'output_mtg', variety)
...
def get_option_glm_mtg_repository(variety = 'cogshall', 
                                  ...
                                  optionname = None):
    path = get_simulated_mtg_repository(variety)
    if not estimationtype is None:
        path = join(path, GlmTypeName[estimationtype]) 
    else:
        path = join(path, 'fixedarchitecture')         
    ...
    if restriction : path = join(path,RestrictionName[restriction])
    elif not estimationtype is None: path = join(path,'allfactors')
...
def get_glm_mtg_repository(params = dict(), 
    ...
    repparams['estimationtype'] = params.get('GLM_TYPE', eInteractionGlm) if params.get('WITH_GLM', True) else None
...
glmtypeabbrev = {eCompleteGlm : 'c', eSelectedGlm : 's', eInteractionGlm : 'i', None : 'r'}
...
def param_stringid(params, consideronly = None, sep = '_'):
    ...
    if consideronly is None or 'GLM_TYPE' in consideronly:
        ...
        result += glmtypeabbrev[params.get('GLM_TYPE', eInteractionGlm if params.get('WITH_GLM', True) else None)]
    if consideronly is None or 'GLM_RESTRICTION' in consideronly and params.get('WITH_GLM', True):
        if len(result) > 0 : result += sep
...
def stringid_param(string, sep = '_'):
    ...
    if abbrevs[1] == 'r' : params['WITH_GLM'] = False
