...
from openalea.vmango.preparation.doralice_mtg.mtg_import import get_mtg
...
def identify_fruits(lstring):
    ...
    mtgparentid = [vid for vid, nb in list(mtg.property('nb_fr').items()) if mm.get_tree_of_gu(mtg,vid) == tree and nb > 0 and mm.get_unit_cycle(mtg,vid) == 3]
    ...
    c = list(Counter(parentmtgids).items())
    c.sort()
    print('Lstring:',len(c),':',c)
    mtgparentid = [(vid,nb) for vid, nb in list(mtg.property('nb_fr').items()) if mm.get_tree_of_gu(mtg,vid) == tree and nb > 0 and mm.get_unit_cycle(mtg,vid) == 3]
    mtgparentid.sort()
    print('MTG:',len(mtgparentid),':',mtgparentid)
