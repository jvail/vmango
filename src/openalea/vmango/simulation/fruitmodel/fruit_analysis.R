...
all_names <- grep("meanfruit", dir(), value=T)
impor_donnees <- lapply(all_names, read.table, sep=",", dec=".", header=T)
sapply(impor_donnees, dim)
repet <- sapply(impor_donnees, nrow)
tables <- do.call(rbind, impor_donnees)
branche_fruitiere <- rep(1:length(all_names), repet)
tables <- cbind(tables, branche_fruitiere)
tables <- tables[,c(ncol(tables), 1:(ncol(tables)-1))]
...
resum_result <- summarise(by_branche, max_X = max(X)-1,poids_frais_grammes = round(max(Masse_Fruit),2), sucres_solubles = round(sucres_solubles[max(X)-1],2), acides_organiques = round(acides_organiques[max(X)-1],2))
...
write.csv(resultats, file="analyses_donnees.csv")
