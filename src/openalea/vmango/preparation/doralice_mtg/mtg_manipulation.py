...
def use_global_mtg(f):
    def simplified_func(*args, **kwds):
        if not 'mtg' in kwds and len(args) == 0 or type(args[0]) != MTG:
            ...
            return f(*args,**kwds)
    import inspect
    args, varargs, keywords = inspect.getargs(f.__code__)
    if varargs: args.append('*'+varargs)
    if keywords: args.append('**'+keywords)
    doc = f.__name__+'('+','.join(args)+')'
    if f.__doc__:
        doc += '\n'+f.__doc__
...
def get_all_treenames(mtg):
    return [get_tree_name(mtg,tid) for tid in get_all_trees(mtg)]

@use_global_mtg
def get_treenames_of_variety(mtg, variety = 'cogshall', loaded = None):
    return [get_tree_name(mtg,tid) for tid in get_all_trees_of_variety(mtg, variety, loaded)]

@use_global_mtg
def get_all_trees_of_variety(mtg, variety = 'cogshall', loaded = None):
    if loaded is None:
        return [k for k,v in list(mtg.property(VarietyPropertyName).items()) if v == variety]
    else:
        return [k for k,v in list(mtg.property(VarietyPropertyName).items()) if v == variety and load_state(mtg,k) == loaded]
...
def select_trees(mtg, loaded= None, variety = "cogshall"):
  ...
  if loaded is None:
      ...
      else: return [i for i,v in list(mtg.property(VarietyPropertyName).items()) if v == variety]
  else : 
      ...
      if variety == "all" or variety is None: 
            ...
            return [i for i,v in list(mtg.property(VarietyPropertyName).items()) if v == variety and load[i] == loaded_propvalue]
...
def get_sorted_terminal_gu_per_nb_ancestor_within_cycle(mtg, cycles = [4,5], variety = 'cogshall'):
    ...
    for cycle in cycles:
        ...
        nb_ancestors = [nb_ancestors_within_cycle(mtg,x) for x in terminal_gus]
...
def filter_reiterations_and_descendants(mtg, units):
    ...
    return [gu for gu,reit in list(reiterations.items()) if reit is False]
...
def check_terminal_flowering_has_no_apical(mtg, variety = 'cogshall'):
    ...
    for cycle in range(3,6):
        ...
        mixed_inflo = list(filter(is_gu_mixed_inflorescence, lapical_children))
    ...
    nbterminalgus = sum([len(get_terminal_gus_of_variety_at_cycle(mtg, cycle, loaded, variety)) for cycle in range(3,6)])
    ...
    print("It represents",old_div(100.*len(flowering_with_apical),nbterminalgus),"% of the terminal gus.")
    print("The apical children are :",apical_children)
    mixed_inflo = list(filter(is_gu_mixed_inflorescence, apical_children))
    print("The number of gu marked as mixed inflo from this set is",len(mixed_inflo),':',mixed_inflo)
...
def check_terminal_non_flowering_has_no_apical(mtg, variety = 'cogshall'):
    ...
    print("It represents",old_div(100.*len(nflowering_without_apical),nbterminalgus),"% of the terminal gus.")
...
def check_if_has_lateral_has_apical(mtg, variety = 'cogshall'):
    ...
    print("It represents",old_div(100.*len(children_with_no_apical),len(get_all_gus_of_variety(mtg, None, variety))),"% of the gus.")
...
def check_if_within_has_lateral_has_apical(mtg, variety = 'cogshall'):
    ...
    print("It exists",len(children_with_no_apical)," gu with only vegetative lateral children and no apical and no inflorescences:",children_with_no_apical)
    print("It represents",old_div(100.*len(children_with_no_apical),len(get_all_gus_of_variety(mtg, None, variety))),"% of the gus.")
    cycles = list(map(get_unit_cycle, children_with_no_apical))
    print("Cycle:", cycles)
    print('Hist cycle:', np.histogram(cycles, list(range(3,7))))
    ranks = list(map(rank_within_cycle, children_with_no_apical))
    print("Rank:", ranks)
    print('Hist rank:', np.histogram(ranks, list(range(5))))
...
def check_apical_strength(mtg, cycles = [4,5], variety = 'cogshall'):
    ...
    for order, ucs in list(sorted_gus.items()):
        histo = dict()
...
def check_burst_dates(mtg, cycles = [4,5], variety = 'cogshall'):
    ...
    for order, ucs in list(sorted_gus.items()):
        ...
        histo_date = [OrderedDict([(i,0) for i in range(6,13)]+[(i,0) for i in range(1,6)]) for i in range(order+1)]
        for uc in ucs:
        ...
        plot_histo([Month[m] for m in list(histo_date[0].keys())],list(reversed([list(hd.values()) for hd in histo_date])),'Burst date of gu of order '+str(order))
...
def check_produce_within_and_next_cycle(mtg, variety = 'cogshall'):
    ...
    print("It exists", sum([len(reit) for w, reit in list(within_and_next.values())]), " gu that produce both within and in the next cycles.")
    if len(within_and_next) > 0:
        print('The reiteration parent units are', list(within_and_next.keys()))
        code = mtg.property('code')
        pgu = lambda gu : str(get_burst_date(mtg,gu))+'('+str(get_unit_cycle(mtg,gu))+'):'+code[gu]
        for gu, children  in list(within_and_next.items()):
        ...
        for i in range(3,6):
            print(np.histogram(nbdescendants[i], list(range(0,10))))

@use_global_mtg
def check_mixed_inflo(mtg, variety = 'cogshall'):
    from openalea.vmango.utilities.util_date import MonthName
    import numpy as np
    nbchild_mi = {}
    ...
    print('Ids : ',list(nbchild_mi.keys()))
    print('Histogram of mixed inflorescences children :', np.histogram(list(nbchild_mi.values()), list(range(0,10))))
    forbetween = []
    for gu in list(nbchild_mi.keys()):
    ...
    isapical = [is_apical(mtg, gu) for gu in list(nbchild_mi.keys())]
    print('Nb of apical MI  :', sum(isapical), ' from ',len(isapical))

    parents = set([])
    nbmixedinflo = {}
    for gu in list(nbchild_mi.keys()): parents.add(mtg.parent(gu))
    for par in parents:
        nbmixedinflo[par] = len([ch for ch in mtg.children(par) if is_gu_mixed_inflorescence(mtg, ch)])

    print('Nb of MI in siblings  :', np.histogram(list(nbmixedinflo.values()), list(range(0,10))))
    ...
    isloadedtree = [gu for gu in list(nbchild_mi.keys()) if is_loaded(mtg,get_tree_of_gu(mtg, gu))]
    ...
    print('Flowering parents  :', len([gu for gu, nbinflo in list(hasflowering.items()) if nbinflo > 0]), ' from ',len(parents))
    ...
    for par in parents:
        ...
        hasflowering[par] = nbinflo
    print('Flowering parents  :', len([gu for gu, nbinflo in list(hasflowering.items()) if nbinflo > 0]), ' from ',len(parents))
    print('Histogram of nb of inflo of parents :', np.histogram(list(hasflowering.values()), list(range(0,10))))
    ...
    print('Vegetative parents  :', len([gu for gu, nbguchild in list(hasvegdev.items()) if nbguchild > 0]), ' from ',len(parents))
    print('Histogram of nb of gu of parents :', np.histogram(list(hasvegdev.values()), list(range(0,12))))

    production = {}
    for par in parents: 
        production[par] = mtg.nb_children(par)
    print('Producing parents  :', len([gu for gu, nbchild in list(production.items()) if nbchild > 0]), ' from ',len(parents))
    print('Histogram of nb of child of parents :', np.histogram(list(production.values()), list(range(0,14))))
    ...
    def monthhisto(val):
        values, ranges = np.histogram(val, list(range(1,14)))
        ranges = list(ranges)
        return ' , '.join([str(values[ranges.index(r)]) for r in list(range(6,13))+list(range(1,6))]) + " -> "+ ' , '.join([MonthEnName[r] for r in list(range(6,13))+list(range(1,6))])


    for cycle in [4,5]:
        print("* Cycle",cycle,':')
        print('Number of mixed inflos :', len([v for k,v in list(nbchild_mi.items()) if get_unit_cycle(mtg,k) == cycle]))
        ...
        print('Histogram of children :', np.histogram([v for k,v in list(nbchild_mi.items()) if get_unit_cycle(mtg,k) == cycle], list(range(0,10))))
        print("Burst rate :", len([v for k,v in list(nbchild_mi.items()) if get_unit_cycle(mtg,k) == cycle and v > 0])/float(len([v for k,v in list(nbchild_mi.items()) if get_unit_cycle(mtg,k) == cycle])))
        print("Mean nb child :", np.mean([v for k,v in list(nbchild_mi.items()) if get_unit_cycle(mtg,k) == cycle and v > 0]))
        mi_month = []
        for gu in list(nbchild_mi.keys()):
        ...
        print('Delta Month of children :', np.histogram(children_deltamonth, list(range(1,8))))
...
def check_fruit_production(mtg, variety = 'cogshall'):
    ...
    fruits = [(vid,nb,wgt_fr[vid]) for vid, nb in list(mtg.property('nb_fr').items()) if get_tree_of_gu(mtg,vid)in trees and nb > 0]
    for tree in trees:
        for cycle in range(3,5):
            ...
            print(get_tree_name(mtg,tree),'\t:',cycle,'\t:',snb, '\t', swgt, '\t', old_div(swgt,snb) if snb > 0 else 0)
        print()
