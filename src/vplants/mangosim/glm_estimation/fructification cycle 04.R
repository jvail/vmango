...
# Modèle complet
 ...
 summary(selected_glm.fruiting.loaded)        # P=0.095 for Nature_Ancestor_V --> withdrawn

   # probas estimées par combinaison de modalités des facteurs retenus
 fruiting_within_04 <- determine_table_prob_from_glm(selected_glm.fruiting.loaded)
 fruiting_within_04 <- chgt_character(fruiting_within_04)
 
  # effectifs des données dans chaque modalité croisée des facteurs retenus
 size_fruiting_within_04 <- effectifs(selected_glm.fruiting.loaded, data=data, subset=index_fruiting.loaded)
 size_fruiting_within_04 <- chgt_character(size_fruiting_within_04)
 
 # rajout des effectifs dans la table des probas estimées par le glm
  ind <- match(fruiting_within_04$codecomb, size_fruiting_within_04$codecomb, nomatch=NA)
  fruiting_within_04$nb <- size_fruiting_within_04$Freq[ind]
   
 # Exportation du fichier
 write.csv(fruiting_within_04, file="fruiting_within_04.csv", row.names = FALSE)
...
####################################################################################################################################
 ...
 summary(manual_glm.fruiting.loaded)        
   ...
   # probas estimées par combinaison de modalités des facteurs retenus
 fruiting_within_04_manual <- determine_table_prob_from_glm(manual_glm.fruiting.loaded)
 fruiting_within_04_manual <- chgt_character(fruiting_within_04_manual)
 
  # effectifs des données dans chaque modalité croisée des facteurs retenus
 size_fruiting_within_04_manual <- effectifs(manual_glm.fruiting.loaded, data=data, subset=index_fruiting.loaded)
 size_fruiting_within_04_manual <- chgt_character(size_fruiting_within_04_manual)
 
 # rajout des effectifs dans la table des probas estimées par le glm
  ind <- match(fruiting_within_04_manual$codecomb, size_fruiting_within_04_manual$codecomb, nomatch=NA)
  fruiting_within_04_manual$nb <- size_fruiting_within_04_manual$Freq[ind]
   
 # Exportation du fichier
 write.csv(fruiting_within_04_manual, file="fruiting_within_04_manual.csv", row.names = FALSE)
...
###  AUTRES ANALYSES
#################################
 index_fruiting.loaded.dec.A <- which(data$Flowering == 1 & data$Tree_Fruit_Load == 1 & data$Burst_Date == 3 & data$Position_A == 1)
 data$Fruiting[index_fruiting.loaded.dec.A]
