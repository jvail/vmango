...
# Modèle complet
 ...
 nb_fruits_within_05 <- determine_table_prob_from_glm(selected_glm.fruiting.loaded)
 nb_fruits_within_05 <- chgt_character(nb_fruits_within_05)
 
  # effectifs des données dans chaque modalité croisée des facteurs retenus
 size_nb_fruits_within_05 <- effectifs(selected_glm.fruiting.loaded, data=data, subset=index_fruiting.loaded)
 size_nb_fruits_within_05 <- chgt_character(size_nb_fruits_within_05)
 
 # rajout des effectifs dans la table des probas estimées par le glm
  ind <- match(nb_fruits_within_05$codecomb, size_nb_fruits_within_05$codecomb, nomatch=NA)
   nb_fruits_within_05$nb <- size_nb_fruits_within_05$Freq[ind]
   
 # Exportation du fichier
 write.csv( nb_fruits_within_05, file= "nb_fruits_within_05.csv", row.names = FALSE)


##############################################################################################################

 # sélection manuelle des facteurs on garde ceux en dessous de P=0.05
 manual_glm.fruiting.loaded <- glm(nb_fruits ~ Nature_Ancestor_V + Flowering_Week + Position_A,
                                family = poisson, data=data, subset = index_fruiting.loaded)

 summary(manual_glm.fruiting.loaded)        

   # probas estimées par combinaison de modalités des facteurs retenus
 nb_fruits_within_05_manual <- determine_table_prob_from_glm(manual_glm.fruiting.loaded)
 nb_fruits_within_05_manual <- chgt_character(nb_fruits_within_05_manual)
 
  # effectifs des données dans chaque modalité croisée des facteurs retenus
 size_nb_fruits_within_05_manual <- effectifs(manual_glm.fruiting.loaded, data=data, subset=index_fruiting.loaded)
 size_nb_fruits_within_05_manual <- chgt_character(size_nb_fruits_within_05_manual)
 
 # rajout des effectifs dans la table des probas estimées par le glm
  ind <- match(nb_fruits_within_05_manual$codecomb, size_nb_fruits_within_05_manual$codecomb, nomatch=NA)
  nb_fruits_within_05_manual$nb <- size_nb_fruits_within_05_manual$Freq[ind]
   
 # Exportation du fichier
 write.csv(nb_fruits_within_05_manual, file="nb_fruits_within_05_manual.csv", row.names = FALSE)
                                        

 index_loaded <- which(data$Tree_Fruit_Load == 1)
table(data$Fruiting[index_fruiting.loaded.dec.A])
length(index_fruiting.loaded.dec.A)
...
pch = 19)
legend("topright", legend =paste("R²=",resume[8])) #pour calculer r²
#abline(1.050, 0.184, col = 'red')		  #tracer la droite de régression 
...
index <- which(data$Position_A == 0 & data$Nature_Ancestor_V == 0)
table(index)
